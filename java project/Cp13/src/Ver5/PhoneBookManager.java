...
//			}
	...
	void deleteInfo() {
		...
		books.indexOf(name);
		if(books.indexOf(name)
				
				) {
			System.out.println("찾으시는 이름이 없습니다.");
		}else {//있다면 시프트하기. 넘오브만큼 1감소하기
			//삭제 위치에서 왼쪽으로 시프트처리
			for(int i=books.indexOf(name); i<books.size(); i++) {
...
//			int editname = books.indexOf(name);
//			
...
//			
//			
//			if(books.get(editname) instanceof PhoneUnivInfor) {
...
//				
//			}else if(books.get(editname) instanceof PhoneCampanyInfo) {
//				System.out.println("회사이름을 입력해 주세요");
//				String company = kb.nextLine();
//				System.out.println("부서를 입력해 주세요");
//				String dept = kb.nextLine();
